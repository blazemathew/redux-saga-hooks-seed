import resourceUrl from './makeUrl';
...
export const generateUrlFromContainerWidth = (
  ...
  const actualWidth = Array.from(imageWidths, ([, value]) => value).reduce(
    (prev, curr) => {
      if (prev) {
        return Math.abs(intrinsicWidth - curr) < Math.abs(intrinsicWidth - prev)
          ? curr
          : prev;
      }
      return curr;
...
export const generateSrcset = (imageURL, type, ratio) => {
  ...
  return Array.from(imageWidths, ([, value]) => value)
    .map(
      (width) =>
        `${generateSrcsetUrl(width, Math.round(width / imageRatio))} ${width}w`
    )
    .join(',\n');
};
